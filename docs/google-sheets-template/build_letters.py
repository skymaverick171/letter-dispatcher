...
# Same brand palette as build.py (the Contacts template) — kept in sync by
# eye since these are two small, independent generator scripts rather than
# a shared module, matching how the rest of this project's per-file build
# scripts (icons.js, build_i18n.js, ...) are organized.
ACCENT = "1E4B8F"
ACCENT_SOFT = "E3EAF6"
INK = "1F2A24"
MUTED = "5C6A63"
...
headers = ["Subject", "Body", "Cc", "Bcc", "Personalize"]
col_widths = [28, 60, 26, 26, 14]
...
# Same two example letters the app itself ships with (LOCAL_LETTERS in
# app/index.html), so a fresh import lines up with what people already see
# in the app rather than introducing unfamiliar sample text.
examples = [
    [
        "Payment Reminder (example)",
        "Hello, {{name}}!\n\nJust a reminder that your invoice is due in 3 days. If you've already paid, please disregard this message.\n\nThanks!",
        "", "", "yes",
    ],
    [
        "Meeting Invitation (example)",
        "Hi {{name}},\n\nYou're invited to a meeting this Thursday at 3:00 PM. Please let us know if that time works for you.\n\nBest regards",
        "", "", "yes",
    ],
]
...
# Personalize is a yes/no suggestion, same spirit as the Group dropdown in
# the Contacts template (a hint, not a strict rule — the app accepts any
# other value there as "yes"/personalized).
dv = DataValidation(
    type="list",
    formula1='"yes,no"',
    allow_blank=True,
...
dv.error = "Type yes or no — anything else (or blank) is treated as \"yes\" (personalized) when imported."
...
RU = dict(
    ...
    s2_body=(
        "1) Выделите заполненные строки в Letters (можно вместе с заголовком — приложение само его распознает).\n"
...
EN = dict(
    ...
    s1_body=(
        ...
        "The letter text (Body) can use the {{name}} and {{email}} placeholders — the app fills them "
        "in with each recipient's details when sending.\n"
    ...
    s2_body=(
        "1) Select the filled-in rows in Letters (you can include the header row — "
    ...
    s3_body=(
        "Recipients aren't part of this template — the same letter is usually sent to different people "
        "at different times, so you pick recipients in the app separately, after importing the letter "
        '(Letters tab → "Recipients for this letter").'
    ),
    s3_h=55,
    callout=(
...
FR = dict(
    ...
    subtitle="Comment utiliser cette feuille comme ensemble de lettres prêtes à l'emploi pour l'application Letter Dispatcher.",
    ...
    s1_body=(
        ...
        "Le texte de la lettre (Body) peut utiliser les balises {{name}} et {{email}} — l'application "
        "les remplace par les informations de chaque destinataire lors de l'envoi.\n"
        "Les lignes marquées « (example) » sont des exemples — supprimez-les ou remplacez-les par vos "
        "propres données."
    ),
    s1_h=110,
    ...
    s3_body=(
        "Les destinataires ne font pas partie de ce modèle — la même lettre est généralement envoyée à "
        "des personnes différentes à des moments différents, donc vous les choisissez séparément dans "
        "l'application, après avoir importé la lettre (onglet Letters → « Recipients for this letter »)."
    ),
    s3_h=65,
    callout=(
